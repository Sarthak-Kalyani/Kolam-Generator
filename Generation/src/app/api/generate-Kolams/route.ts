...
export async function GET(req: Request) {
  try {
    ...
    // ✅ Convert SVG → PNG so React Native Image can display it
    const pngBuffer = await sharp(Buffer.from(svgContent)).png().toBuffer();

    // ✅ Save PNG file to shared /photos volume
    ...
    console.log(`💾 Saved Kolam PNG: ${filePath}`);
