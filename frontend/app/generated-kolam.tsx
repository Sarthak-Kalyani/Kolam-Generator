  ...
  const handleSaveToGallery = async () => {
    ...
    try {
      ...
      if (uriToSave.startsWith("file://")) {
        ...
        assetUri = uriToSave;
      } else if (uriToSave.startsWith("data:")) {
        // data:image/...;base64,XXXX  -> write to temp PNG file
        const base64Data = uriToSave.split(",")[1] ?? "";
        if (!base64Data) {
          throw new Error("Invalid generated kolam image data.");
        }
        const fileUri = `${FileSystem.cacheDirectory}generated_kolam_${Date.now()}.png`;
        ...
        const filename =
          uriToSave.split("/").pop() || `generated_kolam_${Date.now()}.png`;
